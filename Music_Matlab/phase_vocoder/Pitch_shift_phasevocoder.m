...
N = 2048; HA = N/8; interval = -5; s = 2^(interval/12); % select Window Size, Analysis Hop size and ratio Q. For best results take 0.2 < Q < 2.

warning('N must be a power of 2, Q must be a value between 0.2 and 2'); 
...
subplot(2,2,1); plot(x); ylabel('Amplitude'); xlabel('Time (s)') 
subplot(2,2,3); plot(outputy); ylabel('Amplitude'); xlabel('Time (s)')
