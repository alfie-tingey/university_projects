...
class GaussianProcess(object):
    ...
    def get_negative_log_marginal_likelihood(self,
        ...
        K = self._covariance_matrix + sigma2_n*np.identity(self._covariance_matrix.shape[0])

        y = self._array_objective_function_values
        y_transpose = y.T

        inv_term = (1/2)*(y_transpose.dot(np.linalg.inv(K)).dot(y))

        det_term = (1/2)*np.log(np.linalg.det(K))

        n = K.shape[0]

        constant = (n/2)*np.log(2*np.pi)

        negative_log_likelihood = (inv_term + det_term + constant).flatten()

        return negative_log_likelihood[0]
    ...
    def get_sample(self,
        ...
        """
        # TODO
        noise_scale = np.exp((self._kernel.log_noise_scale))**2

        size = self._covariance_matrix.shape[0]

        mean = self._kernel(new_data_points,self._array_dataset).dot(np.linalg.inv(self._covariance_matrix + noise_scale*np.identity(size))).dot(self._array_objective_function_values)
        variance = self._kernel(new_data_points,new_data_points) - self._kernel(new_data_points,self._array_dataset).dot(np.linalg.inv(self._covariance_matrix + noise_scale*np.identity(size))).dot(self._kernel(self._array_dataset,new_data_points))

        mean = mean.flatten()

        return np.random.multivariate_normal(mean, variance).T.flatten()
    ...
    def get_gp_mean_std(self,
        ...
        standard_dev = np.sqrt((np.diagonal(variance)))
    ...
    def get_log_predictive_density(self,
        ...
        covariance = self._kernel(data_points_test,data_points_test) - self._kernel(data_points_test,self._array_dataset).dot(np.linalg.inv(self._covariance_matrix + noise_scale*np.identity(size))).dot(self._kernel(self._array_dataset,data_points_test))

        covariance = np.diag(np.diag(covariance)) + noise_scale*np.identity(size)

        print(mean.shape)

        sum = np.sum(np.log(scipy.stats.multivariate_normal.pdf(evaluations_test,mean,covariance)))
        lpd = sum

        # return float not an array type
