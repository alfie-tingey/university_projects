...
def get_log_upper_proba_distribution_gp(gaussian_process: GaussianProcess,
    ...
    log_marginal_likelihood = gaussian_process.get_log_marginal_likelihood(*theta)
...
def metropolis_hastings_gaussian_process(gp: GaussianProcess,
   ...
   """
    ...
    while len(list_samples) < number_expected_samples:
        ...
        covariance_matrix = np.identity(first_theta.shape[0])*sigma_exploration_mh**2

        newly_sampled_theta = np.random.multivariate_normal(first_theta, covariance_matrix)

        theta_likelihood = get_log_upper_proba_distribution_gp(gp, first_theta)

        theta_new_likelihood = get_log_upper_proba_distribution_gp(gp, newly_sampled_theta)
