...
def expected_log_marginal_likelihood(mu: np.ndarray,
    ...
    for i in range(len(epsilon)):
        theta = mu + np.dot(A,epsilon[i])

        amplitude_gaussian_squared = np.exp(theta[0][0])

        length_scale = np.exp(theta[0][1])

        noise_scale_squared = np.exp(theta[0][2])

        amplitude_linear_squared = np.exp(theta[0][3])

        offset_squared = np.exp(theta[0][4])

        c = np.exp(theta[0][5])

        log_prob_predicted_y = _get_log_marginal_likelihood_gp(amplitude_gaussian_squared, length_scale, noise_scale_squared, amplitude_linear_squared, offset_squared, c, X, y, distances_array)

        expected_log_lhood.append(log_prob_predicted_y)

    return np.mean(np.array(expected_log_lhood).T)
...
def variational_inference_gp(X: np.ndarray,
    ...
    while counter < number_iterations:
        ...
        epsilon = onp.random.multivariate_normal(np.zeros(P), np.identity(P), num_samples_per_turn)

        #############################

        mu_grad = grad(expected_log_marginal_likelihood, 0)(mu, A, epsilon, X, y) - grad(kl_div, 0)(mu, A, sigma_prior)
        A_grad = grad(expected_log_marginal_likelihood, 1)(mu, A, epsilon, X, y) - grad(kl_div, 1)(mu, A, sigma_prior)
