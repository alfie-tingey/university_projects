...
def kl_div(mu: np.ndarray,
    ...
    covariance_post = np.dot(A,A.T)
    mean_post = mu
    size = len(mu)
    mean_prior = np.zeros(shape=(A.shape[0],1))
    covariance_prior = sigma_prior**2*np.identity(A.shape[0])
...
def expected_log_likelihood(mu: np.ndarray,
    ...
    for i in range(len(epsilon)):
        ...
        prob_predicted = sigmoid(X, theta)

        log_prob_predicted_y = y*np.log(prob_predicted) + (y-1)*np.log(1-prob_predicted)

        expected_log_lhood.append(np.sum(log_prob_predicted_y))
...
def variational_inference_logistics(X: np.ndarray,
    ...
    while counter < number_iterations:
        ...
        # elbo = expected_log_like - kl # formula used below

        mu_grad = grad(expected_log_likelihood, 0)(mu, A, epsilon, X, y) - grad(kl_div, 0)(mu, A, sigma_prior)
        A_grad = grad(expected_log_likelihood, 1)(mu, A, epsilon, X, y) - grad(kl_div, 1)(mu, A, sigma_prior)
